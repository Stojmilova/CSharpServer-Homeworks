    ...
    public class SqlServerResponseGenerator : IResponseGenerator, IDisposable
    {
        ...
        private async Task<Response> GetResponse(Request request, ILogger logger)
        {
            ...
            switch (command)
            {
                ...
                case SqlResponseCommand.TableDataID:
                    {
                        var tableName = path.Skip(1).First();
                        var id = int.Parse(path.Skip(2).First());
                        responder = new TableDataID(ConnectionString, tableName, id);
                        break;
                    }
                default:
                    responder = new Error();
        ...
        private SqlResponseCommand GetCommand(IEnumerable<string> path)
        {
            ...
            if (path.Count() == 3)
            {
                ...
                var ID = int.Parse(path.Skip(2).First());
                if ((path.First() == "tables") && (path.Skip(2).First() == $"{ID}"))
                {
                    return SqlResponseCommand.TableDataID;
                }
            }
